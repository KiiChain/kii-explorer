...
const totalPages = computed(() =>
  Math.ceil(smartContractsCount.value / itemsPerPage)
);
...
<template>
  <div class="h-full w-full">
    <div class="brand-gradient-border w-fit my-4 hidden">
      <div
        class="btn bg-base-100 dark:bg-base-100 text-white w-fit border-none"
      ...
      <div
        class="btn bg-radial-gradient-base-duo bg-base-100 dark:bg-base-100 text-white w-fit"
      >
      ...
      <div class="bg-base-100 dark:bg-base100 px-5 py-5 text-white space-y-4">
        <div class="overflow-x-auto">
          <table class="table table-compact w-full mt-5">
            ...
            <tr
              v-for="(contract, i) in latestSmartContracts"
              :key="i"
              class="hover"
            >
              <td>
              ...
              <td>
                <div class="max-w-sm">
                  {{ contract.transaction.accessList?.join(',') }}
                </div>
              </td>
              <td>
    ...
    <div v-else class="h-full w-full flex justify-center items-center">
      <div
        class="px-5 py-5 text-white bg-radial-gradient-base-duo border rounded-lg"
      >
        <h1 class="text-xl font-bold">SWITCH TO KIICHAIN</h1>
        <p class="mt-2">
          Please switch your network to Kiichain to access this feature.
        </p>
      </div>
    </div>
  </div>
