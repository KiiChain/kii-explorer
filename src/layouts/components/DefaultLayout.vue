...
const isKiichain =
  window.location.pathname.search(encodeURIComponent('Testnet Oro')) > -1;

function isNavGroup(nav: VerticalNavItems | any): nav is NavGroup {
  return (
    (nav as NavGroup).children !== undefined &&
    (nav as NavGroup).children.length > 0
  );
}
    ...
    <div class="px-3 pt-4 h-screen flex flex-col">
      ...
      >
        <span class="font-semibold"
          >We have deployed a new testnet. Check out
          <a href="/Testnet Oro" class="underline">Kiichain Testnet Oro</a>
          now.</span
        >
      ...
      <!-- header -->
      <div
        class="flex items-center py-4 rounded px-4 sticky top-0 z-10 rounded-3xl"
        v-bind:style="{
      ...
      <div class="w-full overflow-x-auto overflow-y-hidden">
        <div
          v-for="(item, index) of filteredChainMenu.computedChainMenu"
          :key="index"
          class="px-2"
        >
          <div
          ...
          >
            <div
              v-if="
                index > 0 &&
                index < blockchain.computedChainMenu.length &&
                false
              "
            ...
            >
              ...
              >
                ...
                <Icon
                  v-if="el.icon?.icon"
                  :icon="el.icon?.icon"
                  class="text-xl mr-2 text-accent dark:text-accent"
                />
                <div
                  class="text-base capitalize text-gray-500 dark:text-gray-300 whitespace-nowrap"
                  :class="[
                    {
